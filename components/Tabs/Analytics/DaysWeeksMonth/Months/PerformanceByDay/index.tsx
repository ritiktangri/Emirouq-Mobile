...
export default function PerformanceByDay({ chartId }: any) {
  ...
  const dataAxis = useMemo(
    () => chartReport?.months?.map((item: any) => item.month),
    [chartReport?.months]
  );
  ...
  useEffect(() => {
    if ((!chartId && chartReport?.months?.length === 0) || loadingScreens === 'months') return;
  ...
  }, [chartId, chartReport?.months, loadingScreens]);
    ...
    () => [
      {
        ...
        type: 'bar',
        data: chartReport?.months?.map((value: any) => ({
          value: value?.pnl,
          itemStyle: {
            borderRadius: 3,
            color:
  ...
  useEffect(() => {
    if (!chartInstance || chartReport?.months?.length === 0 || loadingScreens === 'months') return;
  ...
  }, [chartInstance, chartReport?.months?.length, option, loadingScreens]);

  // Enable data zoom when user click bar.
  useEffect(() => {
    if (!chartInstance || !dataAxis || !chartReport?.months?.length || loadingScreens === 'months')
      return;
    ...
    const clickHandler = (params: any) => {
      chartInstance.dispatchAction({
        ...
        endValue:
          dataAxis[Math.min(params.dataIndex + zoomSize / 2, chartReport?.months?.length - 1)],
      });
    };
  ...
  }, [chartInstance, dataAxis, chartReport?.months?.length, loadingScreens]);
