...
export default function DistributionChart({ chartId }: any) {
  ...
  const dataAxis = useMemo(
    () => chartReport?.days?.map((item: any) => item.day),
    [chartReport?.days]
  );
  ...
  }, [chartId, chartReport?.days, loadingScreens]);
  ...
  }, [chartInstance, dataAxis, chartReport?.days?.length, loadingScreens]);
