...
export default function PerformanceByDay({ chartId }: any) {
  ...
  const dataAxis = useMemo(() => chartReport?.days?.map((item: any) => item.day), [chartReport]);
  const skiaRef = useRef(null);
  const { registerChart, unregisterChart }: any = useECharts();
  const [chartInstance, setChartInstance] = useState(null as any);

  useEffect(() => {
    if (!chartId || chartReport?.days?.length === 0 || loadingScreens === 'days') return;
    const currentChart = registerChart(chartId, skiaRef.current, width, 400);
    setChartInstance(currentChart);

    return () => {
      if (currentChart && (chartReport?.days?.length === 0 || loadingScreens === 'days')) {
        unregisterChart(chartId);
      }
    };
  }, [chartId, chartReport?.days?.length, loadingScreens]);
    ...
    () => [
      {
        ...
        type: 'bar',
        data: chartReport?.days?.map((value: any) => ({
          value: value?.pnl,
          itemStyle: {
            borderRadius: 3,
            color:
  ...
  useEffect(() => {
    if (!chartInstance || chartReport?.days?.length === 0 || loadingScreens === 'days') return;
  ...
  }, [chartInstance, chartReport?.days?.length, option, loadingScreens]);

  // Enable data zoom when user click bar.
  useEffect(() => {
    if (!chartInstance || !chartReport?.days?.length || loadingScreens === 'days') return;
    ...
    const clickHandler = (params: any) => {
      chartInstance.dispatchAction({
        ...
        endValue:
          dataAxis[Math.min(params.dataIndex + zoomSize / 2, chartReport?.days?.length - 1)],
      });
    };
  ...
  }, [chartInstance, dataAxis, chartReport?.days?.length]);
