...
export default function PerformanceByDay({ chartId }: any) {
    ...
    () => [
      {
        ...
        type: 'bar',
        data: chartReport?.weeks?.map((value: any) => ({
          value: value?.pnl,
          itemStyle: {
            borderRadius: 3,
            color:
  ...
  }, [chartInstance, chartReport?.weeks?.length, option]);
  ...
  useEffect(() => {
    ...
    const clickHandler = (params: any) => {
      chartInstance.dispatchAction({
        ...
        endValue:
          dataAxis[Math.min(params.dataIndex + zoomSize / 2, chartReport?.weeks?.length - 1)],
      });
    };
  ...
  }, [chartInstance, dataAxis, chartReport?.weeks?.length]);

  return (
    <View className=" flex  flex-col rounded-lg bg-white dark:bg-dashboard_card">
      <View className="px-7 pt-4">
        <Text className="font-poppinsMedium text-lg dark:text-white">PERFORMANCE BY WEEK</Text>
        {selectedDateText}
      </View>
