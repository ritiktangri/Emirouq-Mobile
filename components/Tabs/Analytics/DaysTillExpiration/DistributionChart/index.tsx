...
export default function DistributionChart({ chartId }: any) {
  ...
  const dataAxis = useMemo(
    ...
    [chartReport?.daysTillExpiration]
  );
  ...
  }, [chartId, chartReport?.daysTillExpiration, loadingScreens]);
  ...
  const series = useMemo(
    ...
    [chartReport?.daysTillExpiration?.length]
  );
  ...
  }, [chartInstance, dataAxis, chartReport?.daysTillExpiration?.length, loadingScreens]);
    ...
    <View className=" flex   flex-col rounded-lg bg-white  dark:bg-dashboard_card">
      <View className="px-7 pt-4">
        <Text className="font-poppinsMedium text-lg dark:text-white ">
          TRADE DISTRIBUTION BY DAYS TILL EXPIRATION
        </Text>
        {selectedDateText}
      </View>
