...
export default function PerformanceByDay({ chartId }: any) {
  ...
  }, [chartId, chartReport?.rMultiple?.length, loadingScreens]);
  ...
  useEffect(() => {
    if (!chartInstance || !chartReport?.rMultiple?.length || loadingScreens === 'days') return;
  ...
  }, [chartInstance, dataAxis, chartReport?.rMultiple?.length]);
