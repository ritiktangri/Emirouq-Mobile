...
export default function DistributionChart({ chartId }: any) {
  ...
  const dataAxis = useMemo(
    () => chartReport?.price?.map((item: any) => item.range),
    [chartReport?.price]
  );
  ...
  }, [chartId, chartReport?.price, loadingScreens]);
  ...
  }, [chartInstance, dataAxis, chartReport?.price?.length, loadingScreens]);
