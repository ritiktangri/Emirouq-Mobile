...
export default function PerformanceByDay({ chartId }: any) {
  ...
  const dataAxis = useMemo(() => chartReport?.price?.map((item: any) => item.range), [chartReport]);
  const skiaRef = useRef(null);
  const { registerChart, unregisterChart }: any = useECharts();
  const [chartInstance, setChartInstance] = useState(null as any);

  useEffect(() => {
    if (!chartId || chartReport?.price?.length === 0 || loadingScreens === 'days') return;
    const currentChart = registerChart(chartId, skiaRef.current, width, 400);
    setChartInstance(currentChart);

    return () => {
      if (currentChart && (chartReport?.price?.length === 0 || loadingScreens === 'days')) {
        unregisterChart(chartId);
      }
    };
  }, [chartId, chartReport?.price?.length, loadingScreens]);
  ...
  const tooltip = useMemo(
    () => ({
      ...
      formatter: (params: any) => {
        ...
        if (param?.name === 'Under 2') {
          split = `Under ${toCurrency(param?.name?.split(' ')?.[1])}`;
    ...
    () => [
      {
        ...
        type: 'bar',
        data: chartReport?.price?.map((value: any) => ({
          value: value?.pnl,
          itemStyle: {
            borderRadius: 3,
            color:
  ...
  useEffect(() => {
    if (!chartInstance || chartReport?.price?.length === 0 || loadingScreens === 'days') return;
  ...
  }, [chartInstance, chartReport?.price?.length, option, loadingScreens]);

  // Enable data zoom when user click bar.
  useEffect(() => {
    if (!chartInstance || !chartReport?.price?.length || loadingScreens === 'days') return;
    ...
    const clickHandler = (params: any) => {
      chartInstance.dispatchAction({
        ...
        endValue:
          dataAxis[Math.min(params.dataIndex + zoomSize / 2, chartReport?.price?.length - 1)],
      });
    };
  ...
  }, [chartInstance, dataAxis, chartReport?.price?.length]);

  return (
    <View className=" flex flex-col  rounded-lg bg-white dark:bg-dashboard_card">
      <View className="px-7 pt-4">
        <Text className="font-poppinsMedium text-lg dark:text-white ">PERFORMANCE BY PRICE</Text>
        {selectedDateText}
      </View>
