...
export default function PerformanceByDay({ chartId }: any) {
  ...
  const dataAxis = useMemo(
    () => chartReport?.quantity?.map((item: any) => item.range),
    [chartReport?.quantity]
  );
  ...
  useEffect(() => {
    if ((!chartId && chartReport?.quantity?.length === 0) || loadingScreens === 'months') return;
  ...
  }, [chartId, chartReport?.quantity, loadingScreens]);
  ...
  const tooltip = useMemo(
    () => ({
      ...
      formatter: (params: any) => {
        ...
        } else if (param?.name?.split(' ')?.includes('more')) {
    ...
    () => [
      {
        ...
        type: 'bar',
        data: chartReport?.quantity?.map((value: any) => ({
          value: value?.pnl,
          itemStyle: {
            borderRadius: 3,
            color:
  ...
  useEffect(() => {
    if (!chartInstance || chartReport?.quantity?.length === 0 || loadingScreens === 'months')
      return;
  ...
  }, [chartInstance, chartReport?.quantity?.length, option, loadingScreens]);
  ...
  useEffect(() => {
    ...
    const clickHandler = (params: any) => {
      chartInstance.dispatchAction({
        ...
        endValue:
          dataAxis[Math.min(params.dataIndex + zoomSize / 2, chartReport?.quantity?.length - 1)],
      });
    };
  ...
  }, [chartInstance, dataAxis, chartReport?.quantity?.length, loadingScreens]);
    ...
    <View className=" flex  flex-col rounded-lg bg-white dark:bg-dashboard_card">
      <View className="px-7 pt-4">
        <Text className="font-poppinsMedium text-lg dark:text-white ">
          PERFORMANCE BY QUANTITY TRADED
        </Text>
        {selectedDateText}
      </View>
