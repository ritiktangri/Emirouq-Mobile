...
export default function HighestPerformingSymbol({ chartId }: any) {
  const { chartReport, loadingScreens }: any = useReport();
  const { selectedDateText }: any = useQuery();

  const dataAxis = useMemo(
    () => chartReport?.instrument?.symbolPerformance?.map((item: any) => item.symbolName),
    [chartReport]
  );
  ...
  useEffect(() => {
    if (
      !chartId ||
      chartReport?.instrument?.symbolPerformance?.length === 0 ||
    ...
    return () => {
      if (
        currentChart &&
        (chartReport?.instrument?.symbolPerformance?.length === 0 || loadingScreens === 'weeks')
      ) {
    ...
    () => [
      {
        ...
        type: 'bar',
        data: chartReport?.instrument?.symbolPerformance?.map((value: any) => ({
          value: value?.pnl,
          itemStyle: {
            borderRadius: 3,
            color:
  ...
  }, [chartInstance, chartReport?.instrument?.symbolPerformance?.length, option]);
        ...
        endValue:
          dataAxis[
            Math.min(
              ...
              chartReport?.instrument?.symbolPerformance?.length - 1
            )
          ],
      });
    };
  ...
  }, [chartInstance, dataAxis, chartReport?.instrument?.symbolPerformance?.length]);
