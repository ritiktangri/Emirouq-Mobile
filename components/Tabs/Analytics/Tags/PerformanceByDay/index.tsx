...
export default function PerformanceByDay({ chartId }: any) {
  ...
  const dataAxis = useMemo(
    () => chartReport?.[state?.route]?.map((item: any) => item.tagName),
    [chartReport]
  );
  ...
  useEffect(() => {
    if (!chartId || chartReport?.[state?.route]?.length === 0 || loadingScreens === 'tags') return;
    ...
    return () => {
      if (
        currentChart &&
        (chartReport?.[state?.route]?.length === 0 || loadingScreens === 'tags')
      ) {
        unregisterChart(chartId);
      }
    };
  }, [chartId, chartReport?.[state?.route]?.length, loadingScreens]);
    ...
    () => [
      {
        ...
        type: 'bar',
        data: chartReport?.[state?.route]?.map((value: any) => ({
          value: value?.pnl,
          itemStyle: {
            borderRadius: 3,
            color:
  ...
  useEffect(() => {
    if (!chartInstance || chartReport?.[state?.route]?.length === 0 || loadingScreens === 'tags')
      return;
  ...
  }, [chartInstance, chartReport?.[state?.route]?.length, option, loadingScreens]);

  // Enable data zoom when user click bar.
  useEffect(() => {
    if (!chartInstance || !chartReport?.[state?.route]?.length || loadingScreens === 'tags') return;
    ...
    const clickHandler = (params: any) => {
      chartInstance.dispatchAction({
        ...
        endValue:
          dataAxis[
            Math.min(params.dataIndex + zoomSize / 2, chartReport?.[state?.route]?.length - 1)
          ],
      });
    };
  ...
  }, [chartInstance, dataAxis, chartReport?.[state?.route]?.length]);
  ...
  return (
    <View className=" flex flex-col  rounded-lg bg-white dark:bg-dashboard_card">
      ...
      {chartReport?.[state?.route]?.length === 0 ? <NoData /> : <SkiaChart ref={skiaRef} />}
    </View>
  );
}
