...
import { Image, StyleSheet, View, type ImageStyle, type LayoutChangeEvent } from 'react-native';
import Animated, {
  Easing,
  cancelAnimation,
  useAnimatedStyle,
...
const PinchZoom: React.FC<PinchZoomProps> = ({ uri, containerSize }) => {
  ...
  const pan = Gesture.Pan()
    ...
    .onEnd(({ velocityX, velocityY }) => {
      ...
      translate.y.value = isWithinBoundY.value
        ? withDecay({ velocity: velocityY / 2, clamp: [-1 * boundX, boundX] })
  ...
  const doubleTap = Gesture.Tap()
    ...
    .onEnd((e) => {
      ...
      const higheststImageDimension = Math.max(childWidth.value, childHeight.value);

      const tapOrigin = width > height ? orgnX : orgnY;
      const toScale =
        ((highestScreenDimension + Math.abs(tapOrigin)) / higheststImageDimension) * 2;
      ...
      <GestureDetector gesture={Gesture.Race(pan, pinch, doubleTap)}>
        <Animated.View style={[detectorStyle, styles.center]}>
          <Animated.View style={animatedStyle} onLayout={measureChild}>
            <Image source={{ uri }} style={imageStyle} resizeMethod="scale" resizeMode="contain" />
          </Animated.View>
        </Animated.View>
      </GestureDetector>
    </View>
